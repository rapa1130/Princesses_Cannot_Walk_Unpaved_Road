...
namespace Bisang
{
	...
	void Portal::Update(float dT)
	{
		...
		//Vector2 offset = Vector2::Lerp(m_worldPos + m_startOffset, m_worldPos + m_endOffset, m_t);
		//m_transform->SetPosition(offset);
	}
	void Portal::SetRisingSpeed(float speed)
	{
		m_risingSpeed = speed;
	}
	float Portal::GetRisingSpeed() const
	{
		return m_risingSpeed;
	}
	void Portal::CreateParticle()
	{
		...
		m_spriteRenderer->SetLayer(Layer::Iso);

		m_t = (float)rand() / INT_MAX;

		float minusOneToPlusOne = 2 * ((float)rand() / INT_MAX) - 1;
		float randX = minusOneToPlusOne * m_randRadius;
		minusOneToPlusOne = 2 * ((float)rand() / INT_MAX) - 1;
		...
		m_speed = ((float)rand() / INT_MAX) * m_risingSpeed / 2 + m_risingSpeed / 2;
		m_alphaPhase = 1.0f;
	}
}